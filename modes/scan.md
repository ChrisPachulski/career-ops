...
**Every company MUST have a `careers_url` in portals.yml.** If it does not have one, look it up once, save it, and use it in future scans.

### Level 2 — Greenhouse API (COMPLEMENTARY)

For companies using Greenhouse, the JSON API (`boards-api.greenhouse.io/v1/boards/{slug}/jobs`) returns clean structured data. Use as a quick complement to Level 1 — it is faster than Playwright but only works with Greenhouse.

### Level 3 — WebSearch queries (BROAD DISCOVERY)

The `search_queries` with `site:` filters cover portals across the board (all Ashby, all Greenhouse, etc.). Useful for discovering NEW companies not yet in `tracked_companies`, but results may be stale.
...
The levels are additive — all are executed, and results are merged and deduplicated.
...
7.5. **Verify liveness of WebSearch results (Level 3)** — BEFORE adding to pipeline:
   ...
   **Do not abort the entire scan if a URL fails.** If `browser_navigate` errors out (timeout, 403, etc.), mark as `skipped_expired` and continue with the next one.
...
Every company in `tracked_companies` should have a `careers_url` — the direct URL to its job listings page. This avoids having to look it up every time.

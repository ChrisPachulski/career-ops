...
For each offer: score on every dimension, weighted total score.
Final ranking + recommendation with time-to-offer considerations.

Ask the user for the offers if they are not already in context. They can be text, URLs, or references to previously evaluated offers in the tracker.
